        ...
        <div class="list-rod__detail">
          ...
          <div class="description">
            <p>
              {{
                description
              }}teteteteteteteeteteteteteteteteteteeteteteteteteteteteteetetete
            </p>
          </div>
          ...
          <div v-if="nextPageView" class="next-page-select">
            <div class="content-phase">
              <div class="phase select-info">
                ...
                <div class="text btn" @click="() => setSelectInfo('select01')">
                  作成された議事録
                </div>
                <div class="text btn" @click="() => setSelectInfo('select03')">
                  作成されたエビデンスを確認
                </div>
              </div>
              <div class="phase">
                <div class="item" v-for="item in setImageModeList()">
                  <ItemImager :key="`ItemImager${item.id}`" />
                </div>
              </div>
              <div class="phase">
                <div class="item" v-for="item in setMethodList()">
                  <ItemMethod :key="`ItemMethod${item.id}`" />
                </div>
              </div>
            </div>
